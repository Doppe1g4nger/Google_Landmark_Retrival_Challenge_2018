...
def parse_data(data_file):
    ...
    key_url_label_list = [line[:3] for line in csvreader]
    return key_url_label_list[1:]  # Chop off header


def download_image(key_url):
    out_dir = sys.argv[2]
    (key, url, label) = key_url
    filename = os.path.join(out_dir, label, '{}.jpg'.format(key))
...
def loader():
    ...
    key_url_label_list = parse_data(data_file)
    pool = multiprocessing.Pool()  # Use max CPUs
    failures = sum(tqdm.tqdm(pool.imap_unordered(download_image, key_url_label_list), total=len(key_url_label_list)))
